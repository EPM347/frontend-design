/**
 * Client-side wrapper for the /api/style-match server route.
 *
 * The actual endpoint lives in the main app repo (`best-art-gen-ai-main`).
 * This file is the FRONTEND contract — what the client sends and what
 * shape the server must return. The server-side implementation is
 * documented in the saved Style Match Implementation Brief.
 *
 ...
 *     4. Call Gemini 3.1 Flash Image Preview with the prompt template
 *        for the requested mode + the two reference images
 *     5. Upload result to R2 (key: style-match/{userId}/{nanoid}.png)
 *     6. Insert row into Convex `styleMatchResults` table
 *        (userId, resultUrl, mode, aspectRatio, width, height, createdAt)
...
import type {
  ...
  StyleMatchResult,
  StyleMatchSlot,
} from '~/components/style-match/types';
...
/**
 * Build the multipart payload from the two slots. If a slot has a File,
 * it's sent as binary. If a slot has a remoteUrl, the URL is sent as
 * a text field — server will fetch it on its end (preserves the
 * paste-URL UX without forcing a client-side fetch + CORS dance).
 */
function buildFormData(input: RunStyleMatchInput): FormData {
  ...
  if (input.referenceSlot.file) {
    form.set(
      'referencePhoto',
      input.referenceSlot.file,
      input.referenceSlot.file.name,
    );
...
export async function runStyleMatch(
  input: RunStyleMatchInput,
  ...
  try {
    response = await fetch('/api/style-match', {
      method: 'POST',
      body: form,
      // Auth comes from Clerk session cookie — no header needed.
    });
  } catch (err) {
  ...
  if (!ok.resultUrl || !ok.resultId) {
    throw new StyleMatchError('Server returned an empty result.', {
      refunded: true,
    });
  }
